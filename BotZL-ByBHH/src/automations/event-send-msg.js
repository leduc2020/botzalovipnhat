...
export async function checkAndSendBusinessCard(api, senderId, senderName) {
  ...
  if (!lastSentTime || currentTime - lastSentTime >= BUSINESS_CARD_COOLDOWN) {
    ...
    await api.sendMessage(
      {
        msg:
          `Xin chào ${senderName}! , đây là danh thiếp của chủ tôi:\n` +
          `Link Groups Box Của Tôi \nhttps://zalo.me/g/mlkbfl900\n` +
          `Lưu Ý: Chủ của tôi khong  hỗ trợ các vấn đề liên quan den scam`,
          ttl :500000
      },
      senderId,
...
export async function messagesUser(api, message) {
  ...
  switch (message.type) {
    ...
    case MessageType.GroupMessage: {
      ...
      if (!isSelf) {
        if (threadId == "7828989581970378495") {
